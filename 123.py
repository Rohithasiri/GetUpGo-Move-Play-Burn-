...
if "skip_next_announcement" not in st.session_state:
    st.session_state["skip_next_announcement"] = False
...
# RUN SCHEDULE: pop next item and show announcement (DO NOT start immediately)
    ...
    if not queue:
        ...
        # Only go to announcement if user did NOT press Cancel/Back
        if st.session_state.get("skip_next_announcement", False):
          st.session_state["skip_next_announcement"] = False  # reset flag
        else:
...
# SCHEDULED ANNOUNCEMENT PAGE (shows details and waits for user to start)
    ...
    with col2:
        if st.button("Cancel / Back to Schedule"):
            st.session_state["skip_next_announcement"] = True  # prevent immediate rerun to announcement
...
# RESULT HISTORY
    ...
    if not history:
        ...
        for i, item in enumerate(history[::-1], 1):  # reverse order to show latest first
